...
# Per-user state dict.
# state["state"]:  "new" | "menu" | "catalog" | "course_X_lesson_Y" | "awaiting_payment_X"
# state["tip_index"]: int
# state["completed_courses"]: list of completed course numbers
# state["paid_courses"]: list of paid course numbers
# state["pending_order"]: {order_id, course_num} or None
user_states: dict = {}
...
def get_state(user_id: str) -> dict:
    if user_id not in user_states:
        user_states[user_id] = {
            ...
            "tip_index": 0,
            "completed_courses": [],
            "paid_courses": [1],  # Course 1 is always free/accessible
            "pending_order": None,
        }
    # Migration: add paid_courses for existing users
    if "paid_courses" not in user_states[user_id]:
        user_states[user_id]["paid_courses"] = [1]
    if "pending_order" not in user_states[user_id]:
        user_states[user_id]["pending_order"] = None
...
def _handle_course_selection(state: dict, user_id: str, course_num: int) -> str:
    """Handle course selection — check payment status and start or request payment."""
    price = COURSE_PRICES.get(course_num, 0)
    course_name = COURSE_REGISTRY[course_num]["name"]

    # Free course or already paid → start immediately
    if price == 0 or course_num in state["paid_courses"]:
        state["state"] = f"course_{course_num}_lesson_1"
        return _format_lesson(course_num, 1)

    # MonCash not configured → give access anyway (demo mode)
    if not MONCASH_CLIENT_ID:
        state["paid_courses"].append(course_num)
        state["state"] = f"course_{course_num}_lesson_1"
        return (
            f"🎁 Mòd Demo — Ou gen aksè gratis nan kou \"{course_name}\"!\n\n"
            f"{'─' * 30}\n\n"
            + _format_lesson(course_num, 1)
        )

    # Create MonCash payment
    order_id = f"LA-{user_id[-8:]}-C{course_num}-{int(time.time())}"
    redirect_url = moncash_create_payment(order_id, price)

    if redirect_url:
        # Save pending order
        state["pending_order"] = {"order_id": order_id, "course_num": course_num}
        state["state"] = f"awaiting_payment_{course_num}"
        ...
        return (
            f"💰 Kou \"{course_name}\" koute {price} HTG/mwa.\n\n"
            f"Pou peye ak MonCash, klike sou lyen sa a:\n"
            f"👉 {redirect_url}\n\n"
            f"Apre ou fin peye, retounen isit epi tape \"tcheke\" pou verifye pèman ou.\n\n"
            f"Tape \"menu\" pou retounen san peye."
        )
    else:
        # MonCash API failed — grant access anyway with message
        state["paid_courses"].append(course_num)
        state["state"] = f"course_{course_num}_lesson_1"
        return (
            f"⚠️ Sistèm pèman an pa disponib kounye a. Ou gen aksè gratis tanporè!\n\n"
            f"{'─' * 30}\n\n"
            + _format_lesson(course_num, 1)
        )
...
def process_message(user_id: str, text: str) -> str:
    ...
    # Global payment verification keyword
    if text in ("tcheke", "verifye", "check", "verify") and state.get("pending_order"):
        pending = state["pending_order"]
        order_id = pending["order_id"]
        course_num = pending["course_num"]
        course_name = COURSE_REGISTRY[course_num]["name"]
        if moncash_verify_payment(order_id):
            state["paid_courses"].append(course_num)
            state["pending_order"] = None
            payment_records[order_id]["status"] = "completed"
            state["state"] = f"course_{course_num}_lesson_1"
            sep = '\u2500' * 30
            return (
                f"\u2705 P\u00e8man konfime! Ou gen aks\u00e8 nan kou \"{course_name}\" kounye a.\n\n"
                f"{sep}\n\n"
                + _format_lesson(course_num, 1)
            )
        else:
            return (
                f"\u23f3 P\u00e8man ou poko konfime pou kou \"{course_name}\".\n\n"
                f"Si ou te deja peye, tann k\u00e8k minit epi tape \"tcheke\" ank\u00f2.\n"
                f"Tape \"menu\" pou retounen nan meni prensipal la."
            )
    ...
    if state["state"] == "catalog":
        if text in ("1", "2", "3", "4", "5", "6"):
            course_num = int(text)
            return _handle_course_selection(state, user_id, course_num)
        else:
            return (
    ...
    # ----------------------------------------------------------------
    # STATE: awaiting_payment_X — user must pay before accessing course
    # ----------------------------------------------------------------
    if state["state"].startswith("awaiting_payment_"):
        course_num = int(state["state"].split("_")[-1])
        course_name = COURSE_REGISTRY[course_num]["name"]
        price = COURSE_PRICES.get(course_num, 0)
        return (
            f"Ou gen yon p\u00e8man an atant pou kou \"{course_name}\" ({price} HTG).\n\n"
            f"Tape \"tcheke\" pou verifye p\u00e8man ou.\n"
            f"Tape \"menu\" pou retounen san peye."
        )
...
# ---------------------------------------------------------------------------
# MonCash Payment Callback
# ---------------------------------------------------------------------------

@app.route("/api/payment/callback", methods=["GET"])
def payment_callback():
    """MonCash redirects here after user completes (or cancels) payment."""
    transaction_id = request.args.get("transactionId", "")
    # MonCash sends transactionId as query param after payment
    # We show a simple confirmation page
    return (
        "<!DOCTYPE html><html><head><meta charset='utf-8'>"
        "<meta name='viewport' content='width=device-width, initial-scale=1'>"
        "<title>LakayAprann - P\u00e8man</title>"
        "<style>body{font-family:sans-serif;text-align:center;padding:40px 20px;}"
        "h2{color:#0d6efd;}.box{max-width:400px;margin:0 auto;padding:20px;"
        "border:1px solid #ddd;border-radius:12px;}</style></head>"
        "<body><div class='box'>"
        "<h2>\U0001f4da LakayAprann</h2>"
        f"<p>Tranzaksyon: <strong>{transaction_id or 'N/A'}</strong></p>"
        "<p>\u2705 M\u00e8si pou p\u00e8man ou!</p>"
        "<p>Retounen nan WhatsApp epi tape <strong>\"tcheke\"</strong> "
        "pou aktive kou ou a.</p>"
        "</div></body></html>"
...
def payment_status():
    """Check payment status by order_id (for debugging/admin)."""
    order_id = request.args.get("order_id", "")
    if order_id in payment_records:
        rec = payment_records[order_id]
        return jsonify({
            "order_id": order_id,
